...
| **What was done 2026-09-11** | Contract §1 and legacy item 19 corrected to say what is actually read and that §3.3 applies in full and is unsatisfied. **No compensating heuristic was added, deliberately** |
| **What must happen before an Airtable-imported job drives a rig** | Either the extractor is fixed upstream and re-verified, **or** §3.3's verification is implemented and made mandatory on import. Until one of those, an imported static or cyclic job is not safe to run and a locally-entered one is |
...
| **TC5** | **MU — the operator interface. 🔴 THE RELEASE BLOCKER, and externally owned.** Every backend surface it needs exists and is demonstrated on the real wire; **none of it has a consumer.** Verified 2026-09-11 against the deployed bundle in `src/ifet_ui_react/static/`: it contains no reference to `manual-tests`, `impact-tests`, `ANSI Z97`, `Forced Entry` or `sync/status`. The UI source is not in this repository — `src/ifet_ui_react/` holds a compiled build only — so this cannot be closed here, and writing a second interface alongside the developer's would be a new product surface, not a minimum fix. Three of the five test types therefore have a complete, probed API and **no way for an operator to run them**, which is DG5's original sentence exactly: *"the release was an API with no consumer."* **Now also carries DG14's §3.3 verification**, which has nowhere to live either | DG5 → MU · DG14 | **the UI developer** | ✅ TB3, ✅ TC1c | An operator completes all five test types end to end, sees the sync status of each, and confirms the design-pressure pair before an imported job can run |
