...
| | Consequence |
|---|---|
| ~~No correction route~~ | ✅ **Built 2026-09-08** — `POST /test-results/{id}/correct`. Not deployed |
...
passed 339 tests. Any code path that reaches Airtable needs either a live probe
...
| **A mis-recorded impact is expressible, and now buildably so** | The mechanism is `Corrects Attempt ID` — superseding a record that was *wrong* — which keeps a new firing and a mis-typed outcome cleanly different, as §2's "never disguise a correction as a retest" requires. **TC1g was built first, 2026-09-08, for exactly this reason:** with no edit or delete route for an attempt or a shot, and attempts append-only by §4.5 rule 5, the correction chain is the only fix path that exists even in principle once each impact *is* an attempt |
...
|---|---|
| ~~`contract/write-contract-v0.4.md` §2~~ → `contract/field-register.csv` | ✅ **Corrected 2026-09-08.** The rule lived in the register, not the prose contract — this row said otherwise and was wrong. *"Run ordinal for the programme. **Never a stage index, never an impact ordinal**"* now reads: the attempt ordinal within a test, never a stage index, and **for Impact it is the impact ordinal**, with Static and Cyclic keeping stages as children of one attempt so it stays their retry ordinal |
...
| **DG8** | Forced Entry / ANSI / failure-note output shape | 🔴 **REOPENED by the product owner 2026-09-10** | LabOS | — | Question 2 came back **NO** — *"they are different under different standards"*. `Forced Entry Result` and `ANSI Result` are to be built, **alongside** `Test Result` rather than instead of it. `Failure Notes` stays as it is |
...
> **This is the one finding of the production-readiness pass that changes a decision rather than a
> document.** It is not new *behaviour*; it is a claim in the authoritative contract that stopped being
> true on 2026-09-08 and was not noticed, because the sentence and the code were reviewed on different
> days.

Contract §1 (A9) said, and until 2026-09-11 still said:

> *"LabOS reads the minimum needed to identify work, and no requirement values at all … **This removes the
> extraction defect from the execution path entirely.** … Fields such as `Required Value Inward` remain in
> both bases and are marked `IGNORED` in the register: they exist, and LabOS does not read them."*

**None of that is true now.** The eight typed fields were applied to `Protocol Sections` on 2026-09-08 and
`mirror.py` was wired to them the same day. `mirror.SECTION_FIELDS` is **ten** fields and includes
`Required Value Inward` and `Required Value Outward`; `importer.plan` reads the pair; `create_project_body`
stores it as the project's design pressures; and LabOS derives **all fourteen** static and cyclic stages
from it. The register has said `READ` on those rows since the day they were applied — the register was
right and the contract prose was stale.

So the §10.19 chain is live again: their PDF extractor reads *values in order* rather than *values by
column*, a blank cell slides everything after it one place left, and a 60 PSF requirement arrives as 9.
**A shifted pair is a shifted test**, and the TA6 probe's own hierarchy demonstrates the mechanism in
miniature — 60/45 on a section became six static and eight cyclic stages with no human in between.

| | |
|---|---|
| **Blast radius** | **Static Load and Cycles**, for any job imported from Airtable. Both are derived from the pair and nothing else |
| **Not affected** | **Impact**, since TA7 — the classification is LabOS-owned and a pick from a closed set of three, and a pick cannot shift a column. Forced Entry and ANSI Z97.1 have no numeric requirement at all |
| **Owner of the defect** | **Airtable / IFET.** It is their extractor and their base. LabOS cannot detect it: every shifted value is individually plausible, so there is no range check or heuristic that would be anything but a new way to be confidently wrong |
| **Owner of the control** | **LabOS.** Contract §3.3 already specifies it — the operator enters the independently verified pair with its reference, verifier and time |
| **Why the control does not exist** | A8 deferred the verification UI, and DG5 reduced MU on the strength of A9's "no requirement values are read". That reasoning is void now. `POST /airtable/import/plan` shows `design_pressures` and `refused` before anything is created, so the **information** is available — what is missing is a screen, a required confirmation, and a record of who gave it |
| **What was done 2026-09-11** | **The LabOS half is CLOSED.** Contract §1 and legacy item 19 corrected; then `app/airtable/release.py` built and made mandatory on the backend start path, which is where §3.3 says to check it. Migration `e2b9d4c70a15`, 24 tests, and four assertions on the real wire in the TA6 probe. **No compensating heuristic was added, deliberately** |
| **The control, and why it is the only one that can work** | Not inspection — a shifted 9 PSF validates, is the right kind and carries the right unit, and a test asserts exactly that. **Two independent readings of the same fact must agree**: what LabOS mirrored, and what a named person read off the proposal with its reference and the time. A disagreement is **refused and not stored**, and the imported pair is not overwritten: LabOS does not choose between two contradicting sources, because running on the operator's numbers while Airtable says something else publishes a result claiming a requirement it was not run against |
| **Still open, and externally owned** | **The defect itself.** The extractor still shifts, so an imported job now *stops* instead of running wrong — which is the safe failure, not a fix. Airtable/IFET own fixing it and re-extracting the affected production data. Until then every imported static or cyclic job costs an operator a manual verification, and that cost is the mitigation working |
| **And the UI half** | The verification has **no screen** — it is TC5's screen 2. So today the control is enforceable only through the API, which is the same blocker TC5 already is |

**This does not block deploying the backend.** It blocks *using* the Airtable-import path for static and
cyclic work on a rig. A LabOS-only job, set up by an operator as today, is unaffected — which is exactly
...
That was true until 2026-09-08, when the route was built (TC1g); the field is now blank on every ordinary attempt and every retest, and populated only on a correction. It remains *why* the
...
### DG8 · Forced Entry, ANSI and failure notes — output shape — **REOPENED by the product owner 2026-09-10**

> **Reopened 2026-09-10, and this time by the person entitled to reopen it.** Question 2 of the approval document came back **NO**: *"I would think NO since they are different under different standards"*. So `Forced Entry Result` and `ANSI Result` **are** built, as dedicated `singleSelect` fields populated only for their own `Test Type` and blank on the other four.
>
> **`Test Result` stays, and the two new fields sit alongside it.** It is written at create (`Pending`), terminal and verdict for all five types — it is the attempt's lifecycle state as much as its verdict, and a type that stopped writing it would vanish from every status view in the base. The shape is not new either: **`Impact Result` is already exactly this**, a dedicated per-type result field beside the shared one, so Forced Entry and ANSI are joining a pattern rather than starting one.
>
> `Failure Notes` is **unchanged** — his answer named Forced Entry and ANSI, and we do not add a third field on inference.
>
> Coupled change, and it moves as one or not at all: two register rows, `contract.FIELDS`, `mapping.py`, `preflight.ADDED` 18 → 20, `apply_schema` on the Testing Base 160 → 162, and the change document. `check_register.py` check 4 fails while the register and `preflight.ADDED` disagree. Tracked as **TA6**.
>
> The A9 wording below is what it superseded. The standing rule — *add one only when a named operational report requires it* — was ours, and it was there to stop **us** inventing fields; it was never a veto on the product owner asking.
...
|---|---|---|---|---|
| ~~**1**~~ | ~~**Send the five-test approval document**~~ **✅ SENT 2026-09-08** | TA5a | you | `../correspondence/sent/2026-09-08-po-five-test-approval-request.md`. **Three of the six questions were asked** — 2, 3 and 4; ANSI ordering (1) and `STATIC_PROGRAMME` (5) dropped out of the summary and stay open. **One statement needs correcting in the reply thread:** it tells him Airtable receives deflection data, which it does not and cannot — the Airtable team is being told the opposite in the change document §4 |
| ~~**2**~~ | ~~**The correction route**~~ **✅ DONE 2026-09-08** — `POST /test-results/{id}/correct`, 22 tests. `_typed_attempt` was needed because the attempt subclasses have no polymorphic discriminator, so a base-class query cannot name the parent test | TC1g | LabOS | **Do this before step 4, not after.** There is no shot edit or delete route — `record_shot`, `list_shots`, `add_shot_photo` and nothing else — so a mis-recorded impact cannot be corrected at all today. Attempts are append-only by §4.5 rule 5, so once each impact *is* an attempt, the correction chain is the only path that exists even in principle. Build the redesign first and the invariant reads complete while a mis-typed impact still has nowhere to go |
...
| ~~**3b**~~ | ~~**Fix `impact_count`**~~ **✅ DONE 2026-09-08** — accumulated across impact sections. Summed rather than refused: LMI and SMI are different missiles with their own counts, unlike two design-pressure pairs which cannot both hold | TC1h | LabOS | `importer.py:166` has no accumulation and no conflict check across `IMPACT_LMI`/`IMPACT_SMI`, unlike `design_pressures` directly above it. Small, and it is the five-required/five-rows cross-check the product owner's confirmation just made meaningful |
| ~~**4**~~ | ~~**TC1h — one attempt per impact**~~ **✅ CODE + MIGRATION DONE 2026-09-08** — ordinals, `Impact Result`, the finish gate deriving the attempt outcome from its impact, migration `c7e4a2b81f56` rehearsed forward and back. The rehearsal caught a real collision: renumbering 1..N while other attempts of the same test hold those numbers violates `uq_test_results_test_attempt` on exactly the multi-attempt shape that makes renumbering necessary | TC1h · §4.5a | LabOS | The redesign. In this order inside the step: the ordinals (`record_shot` sets `shot_number = attempt.trial_number`, `_impact_result` reads `trial_number`), then `Impact Result`'s single-impact line and the dead `Incomplete` branch, then the routes, then the migration with its five traps, then the ~70 shot references across seven test files. **Rehearse the migration on a disposable Postgres from a dump before the node sees it** — 39 tests and 114 shots are real |
| ~~**5**~~ | ~~**The document deltas**~~ **✅ DONE 2026-09-08** — register row, spec 160 rows, interface schema regenerated, and the change document's five touch points. A last-round review found the eighteenth field had **no writer**: applied to the base and documented as populated, while `contract.FIELDS` and `mapping.py` had no key and the envelope would have refused one. Wired and tested | TC1h · TA5b | LabOS | Register row for `Impact Number`; regenerate `production-change-spec.csv` and `interface-schema.csv`; `preflight.ADDED` 17 → 18; the change document's five touch points (§4.5a). **Includes one correction owed regardless of any of this:** lines 480–483 tell the Airtable team our mapping points `Impact Velocity` at a measured per-impact value, which is not true |
| **6** | **🟡 READY TO SEND 2026-09-11 — no longer blocked by us.** ~~Blocked on TA7 and TA6~~ **both closed and verified in Testing.** The document is current against both live bases: preflight green with **no warnings**, 164 / 142 / delta 22, and every count in it regenerated. **Superseded as the thing we send, 2026-09-11:** the canonical external package is now `../correspondence/LabOS-Airtable-Production-Schema-Requirements-2026-09-11.md` plus its generated CSV, which absorbs this document's schema content, the ownership boundary and the automation impact into one artefact. This file stays as the historical change reference. The only thing still holding it is the TA5a approval decision of 2026-09-08, which is **yours**, not a build dependency. Still send `../correspondence/po-followup-2026-09-08.md`'s content — folded into `../correspondence/po-answers-and-impact-remap-2026-09-10.md`, which is the message to send now | TA5b · TA6 · TA7 | **you** | `preflight.py` green against both live bases, then send. Carries TC1i's question about their six writable `LabOS`-named fields on `Protocol Sections`, so it goes as one exchange rather than two |

**Not on this path, and deliberately:** ~~TC1f waits on question 3 coming back~~ **closed 2026-09-10**; TC5, the operator interface, is
...
| ~~**TA2**~~ | ~~Add three Impact requirement fields to the Testing Base~~ **✅ 2026-09-08** | §9 | LabOS | — | Applied. **156 → 159.** `Missile Type` `fld5Bs0aQXXeVso2y`, `Missile Weight` `fldmhdhonyyLcx4Ex`, `Impact Velocity` `fldJNfUVyqQEFOVWx`. `Impact Locations` deliberately not created — location is a per-impact observation, not a requirement. Production untouched at 142. **⚠️ Two of the three are superseded 2026-09-10** — the product owner derives weight and velocity from the missile designation, so only the designation survives and it reverses direction. See TA7. Nothing to undo in a live base: all three are Testing Base only |
...
| ~~**TA5a**~~ | **✅ SENT 2026-09-08 · ✅ ANSWERED 2026-09-10.** All four questions in the sent message came back — 1 **YES**, 2 **NO**, 3 *"We will be entering it"*, 4 confirmed — plus an unprompted remark that collapses the Impact read surface. Decode, corrected mapping and the reply: `../correspondence/po-answers-and-impact-remap-2026-09-10.md`. **Two consequences carry work: TA6** (DG8 reopened) **and TA7** (the Impact remap, which blocks TA5b). **Question 5 was never actually asked** — the `sent/` record is wrong about the numbering; all four went, and `STATIC_PROGRAMME` was only ever in the unsent follow-up. It is re-asked in the reply. **Get the five tests approved by the project owner** — `../correspondence/five-test-requirements-approval-2026-09-08.md`. One page per test: what the proposal must supply, what LabOS derives, what is recorded, what reaches Airtable, and what we cannot do yet. **Generated by `app/airtable/test_requirements_doc.py`** from the calculators, the importer's routing, the requirement kinds and the two CSVs — so it cannot describe a field we do not have, and `check_register.py` check 7 fails when it goes stale. **Six** answers requested: ANSI ordering unenforced, the shared result column, the unshown target velocity, impact location staying local, `STATIC_PROGRAMME` changing nothing, and — added 2026-09-08 — confirmation that one attempt per impact means five records in *their* base. **Ready to send; not blocked by TC1h**, because the document states the new model as specified-not-yet-built and the two open decisions inside TC1h are ours rather than his | §3.0 · §10 | **IFET/you** | ✅ TA3, TA4 | Approval recorded, and the answers folded back into the contract before TA5b goes out |
| **TA5b** | **🔴 NOW BLOCKED ON TA7 as well, 2026-09-10.** The document describes four Impact requirement fields; the product owner has just said two of them are redundant and the third points the other way. It cannot go out describing a mapping we have already superseded, and he asked for the missile change to reach the Airtable team — *"We should discuss with AirTabe team"* — which it does, inside this document rather than as a second thread. **Send the Airtable document** — `../correspondence/testing-base-change-document-2026-09-08.md` to the Airtable team. **Written 2026-09-08, preflight clean against both live bases 2026-09-08, NOT SENT — and now held until TA5a is approved (decided 2026-09-08).** It opens with answers to their three clarifications and carries the change reference behind them, so their questions and our changes arrive together. Note that §4 already declares the omissions and offers to add a field only if they name the report needing it, so no answer to TA5a can retract anything in it — the hold buys asking once rather than twice, and costs the Airtable team waiting | §3.0 commitment 3 · §10 | **IFET/you** | ✅ TA5a | Send records filled in and copies in `correspondence/sent/`, which is append-only |
| ~~**TA5c**~~ | **✅ CLOSED 2026-09-11 — regenerated, and it now proposes 19 ADD against production with none of the three withdrawn fields among them.** 161 rows, generated from the live bases; expected production count after promotion **142 → 161**. The sentinel notice is gone because the condition it warned about is gone. Original text below. | **⛔ `production-change-spec.csv` was SUPERSEDED and must not have been applied.** It lists `Missile Type`, `Missile Weight` and `Impact Velocity` as `ADD` against production; all three were withdrawn from the input contract on 2026-09-10. Applying it would put two permanently unread fields in the live base. **Notice applied 2026-09-10** in three places — a sentinel first row in the CSV itself (harmless to `preflight` check 3, which ignores unknown actions), the change document's header, and the generator's docstring. **Not regenerated yet, deliberately:** the generator needs a third action and the register needs the deprecation flip, and both are coupled to the TA7 code — doing it alone leaves `check_register.py` red, which is how the next real drift gets missed. **Production is untouched and stays at 142** | TA7 · PO 2026-09-10 | LabOS | ✅ notice done; regeneration waits on TA7 | The spec is regenerated inside TA7 with no `ADD` row for any of the three |
| ~~**TA7**~~ | **✅ CLOSED 2026-09-11 — implemented, Testing Base schema applied, real-wire probe passed; production deployment pending the normal deployment gate.** The Impact requirement is now the LabOS-owned impact classification. **TA7a** (`a148545`, `03a5a74`): `impact_family` frozen at import from `IMPACT_SMI`/`IMPACT_LMI`, `impact_level` and `target_velocity` operator-entered, one named CHECK, the PATCH route and the completion gate. **TA7b** (`d649d84`/`6946fb7` prepared, `49c3cce`/`b61cc7c` finalised): the three inbound fields withdrawn from the read contract and left physically in Testing, two outbound fields created — `Impact Classification` `fldMY7DiiuP9kbQbL`, `Target Impact Velocity` `fldhywP9YpsmoWWT1`. **Probe** (`fd4dfae`): 64/64 on the real wire. Testing **162**, production **142 and untouched**. Read surface 19 → 16 named fields | §3.1 · §4 · PO 2026-09-10 | LabOS | — | ✅ Done. **Not deployed** — see the deployment gate below |
| **TA7-deploy** | **🔴 The deployment gate, and it is mandatory.** Before the TA7 migration and code reach the running environment, in this order: **(1)** run the read-only open-Impact-attempt check (`tests/check_open_impact_attempts.py`) against the deployment database and record its output in the deployment evidence **even when the answer is zero**; **(2)** rehearse migration `a4f18c2d3b90` forward and back against the deployment DB shape; **(3)** run real `preflight.py` against both bases; **(4)** confirm production Airtable is still **142** and otherwise unchanged. **Do not infer or backfill `impact_family`, `impact_level` or `target_velocity` for historical or open attempts** — nothing is derived from the free-text `missile` column or from past shot velocities. An attempt open at deploy time needs its values set explicitly or the window coordinated around it | TA7 · runbook | LabOS + IFET | a window | All four checks recorded in the deployment evidence, no backfill performed |
| ~~**TA6**~~ | **✅ CLOSED / VERIFIED IN TESTING 2026-09-11 — production deployment pending the same gate as TA7.** `Forced Entry Result` `fldAHuPzZHZEj0Cjt` and `ANSI Result` `fldmCKJV95N9uL7xt` applied to the Testing Base; **162 → 164**, production untouched at **142**. Prepared as `PENDING_SCHEMA` (`c47c246` / `fcb97a7`) so the code could be reviewed before anything was written to a shared base, then finalised (`7ea7cfd` / `7927476`) and probed on the real wire — **97/97 in one clean run** (`d9f6dfe` / `a14b1a2`, evidence `../evidence/ta6-live-probe-2026-09-11/`). The probe proves the JSON-valve transition in both directions: ABSENT preserved the value in `labos_extra` in the **wire** spelling, PRESENT puts it in the column and out of the valve. Four earlier runs failed and **every failure was the probe**, each time because the system correctly refused something — a wrong `Requirement Kind`, an impact attempt with no photograph, and twice a cyclic stage that could not report a cycle count. Original scope below, unchanged. | **Two dedicated result fields — DG8 reopened by the product owner 2026-09-10.** `Forced Entry Result` and `ANSI Result` on the `LabOS Raw Data Table`, `singleSelect`, populated only for their own `Test Type` and blank on the other four. **Alongside `Test Result`, not instead of it** — it carries create/terminal/verdict for all five types, and `Impact Result` is already exactly this shape beside it. `Failure Notes` stays as it is: he named two fields, and we do not add a third on inference. Coupled, and it moves as one: two register rows, `contract.FIELDS`, `mapping.py`, `preflight.ADDED` 18 → 20, `apply_schema` 160 → 162, and the change document. `check_register.py` check 4 is red while the register and preflight disagree | DG8 · PO 2026-09-10 | LabOS | — | Both fields live in the Testing Base, written for their own type only, register and preflight agreeing, checker green |
| **TA7** | **🔴 The Impact remap — agreed with the product owner 2026-09-10, and it blocks TA5b.** The Impact requirement becomes the **impact classification** — SMI, LMI Level D, LMI Level E — selected in LabOS and published with the result; mass and target velocity become parameters of that classification rather than fields anybody supplies. `Required Value` + `Requirement Code` still carry how many impacts, unchanged. **The read surface goes 19 named fields → 16**, and Impact leaves the extractor's blast radius (§10.19) entirely: a pick from a set of three cannot shift a column. Undoes most of TA2 — `Missile Weight` `fldmhdhonyyLcx4Ex` and `Impact Velocity` `fldJNfUVyqQEFOVWx` go unread, and `Missile Type` `fld5Bs0aQXXeVso2y` moves off Protocol Sections to a new `OUT` field on the results table. **Nothing to undo in a live base** — all three are Testing Base only; production is still 142. **Blocked on three answers from him:** is the set exactly three, what mass and target velocity does each carry, and is the classification chosen once per protocol or per test at the rig — the last decides whether it lives on the protocol mirror or on the attempt. **The parameter table is not pre-filled from ASTM E1996 for him to confirm** — once LabOS derives those numbers nobody ever sees them again, so a wrong one is silently ours on every impact test, and it must be his in writing | TA2 · TC1f · PO 2026-09-10 | LabOS + **IFET/you** | his three answers | The classification is stored and published, the two unread fields are `IGNORED` in the register, `mirror.py` allowlists 16, and the parameter table is recorded with his name on it |
...
| **TB4** | **Deploy the three test types to `management`.** Acceptance for the deployed result is `../testing/five-workflow-e2e-acceptance-2026-09-11.md` — backend half green at **60/60** on 2026-09-11, UI half pending TC5 | M3 | LabOS + IFET | TB2, a window | Impact, Forced Entry and ANSI capture live alongside static and cyclic. No Airtable involvement. Runbook `../runbooks/p0-p1-deploy-2026-08-28.md`, **§2 is the go/no-go** |
...
| ~~**TC1f**~~ | ~~**Decide whether the target impact velocity must reach the operator**~~ **✅ CLOSED 2026-09-10 by the product owner** — question 3 came back *"We will be entering it"*, and his remark above the numbered answers says why that is enough: the target velocity follows from the missile designation (SMI has one; LMI is Level D or E). So the operator picking the designation **is** stating the target, and there is nothing to push at them from Airtable. **No new column on `missile_impact_tests` and no screen change.** `Impact Velocity` on Protocol Sections becomes unread — see TA7. `shots.velocity`, the achieved per-shot value, is untouched | reconciliation · `GAP-NOT-SURFACED` · PO 2026-09-10 | LabOS | ✅ TA5a answer | Closed: recorded that the operator does not need a pushed target, because they choose the thing that determines it |
| ~~**TC1g**~~ | ~~**The correction route**~~ **✅ DONE 2026-09-08** — `POST /test-results/{id}/correct`. `corrects_attempt_id` and `correction_reason` had columns, a property and an envelope mapping since P1 and nothing set them, so every attempt was a retest — and with no edit or delete route for an attempt or a shot, a mis-recorded result had no route at all. A correction is a new attempt naming the one it supersedes, reason required, original never edited. Refuses an open original, refuses while another attempt of the test is open, and deliberately works on a *finished* test. 22 tests; `_typed_attempt` added because the attempt subclasses have no polymorphic discriminator | §4 · DG13 | LabOS | — | An operator supersedes a recorded result and Airtable can tell it from a retest |
...
| **TC5** | **MU — the operator interface. 🔴 RELEASE BLOCKER, EXTERNAL. Implementation contract delivered 2026-09-11** — **`../contract/tc5-ui-developer-handoff-2026-09-11.md`** (the earlier `tc5-operator-interface-contract-*` path is now a pointer stub): five screens, and for each one its purpose, routes, request payloads, the response fields that matter, state transitions, operator-entered versus read-only fields, every error and how to display it, completion/abort, photograph behaviour, review, and its acceptance test — plus the twelve-scenario operator acceptance run and an explicit list of what not to build. **Verified route-by-route against the running application 2026-09-11**: all 28 cited routes resolve, and the pass found one stale field name in our own docs (`worker_heartbeat_at`, which never existed — it is `worker_alive` + `heartbeat_age_seconds`). **No REQUIRED INTEGRATION GAP found; the backend stays frozen.** Generated against the live `openapi.json`, which is now gated so it cannot drift from the app. **Screen 2 is new and non-negotiable** — it is DG14's verification, and no imported static or cyclic job runs without it.** Every backend surface it needs exists and is demonstrated on the real wire; **none of it has a consumer.** Verified 2026-09-11 against the deployed bundle in `src/ifet_ui_react/static/`: it contains no reference to `manual-tests`, `impact-tests`, `ANSI Z97`, `Forced Entry` or `sync/status`. The UI source is not in this repository — `src/ifet_ui_react/` holds a compiled build only — so this cannot be closed here, and writing a second interface alongside the developer's would be a new product surface, not a minimum fix. Three of the five test types therefore have a complete, probed API and **no way for an operator to run them**, which is DG5's original sentence exactly: *"the release was an API with no consumer."* **Now also carries DG14's §3.3 verification**, which has nowhere to live either | DG5 → MU · DG14 | **the UI developer** | ✅ TB3, ✅ TC1c | An operator completes all five test types end to end, sees the sync status of each, and confirms the design-pressure pair before an imported job can run |
