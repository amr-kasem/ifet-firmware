...
| **6** | **Preflight, then send the Airtable document** — and send `../correspondence/po-followup-2026-09-08.md` to the project owner, which does not wait on it | TA5b | **you** | `preflight.py` green against both live bases, then send. Carries TC1i's question about their six writable `LabOS`-named fields on `Protocol Sections`, so it goes as one exchange rather than two |
