...
| **14** | **`Corrects Attempt ID` + `Correction Reason` are absent.** These are the only two fields in §4 that the JSON valve cannot rescue, because Airtable automations must *branch* on them — a value buried in long text cannot be filtered, linked, or rolled up. Without them a correction and a genuine retest are indistinguishable in the data (§3.1), so any roll-up counting attempts or computing a pass rate is wrong in one of the two cases. | Airtable | 🔴 **NEW — blocking** **2026-08-31 — they propose replacing this field with the shared `LabOS Test ID`. Declined, with the cost stated.** Grouping is not supersession: the shared Test ID is already in every payload (§2) and answers a different question. Fallback offered — one Airtable formula field over `corrects_attempt_id` in the JSON valve, which makes this depend on **25**. **Still blocking.** `correspondence/airtable-team-questions-2026-08-31.md` §3. |
