...
`docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md`.
...
> **Reconciled again 2026-08-31** against the Airtable team's reply to the 2026-08-28 verification
> report. Their message raises three points, touching items **3**, **8**, **13**, **14**, **15**, **20**,
> **23**, **24** and **25**. Two are accepted, one is declined with the cost recorded; item **20** is
> agreed by them outright. Wording of our answer: `correspondence/airtable-team-questions-2026-08-31.md`.
> **Nothing here is marked closed on the strength of a chat message** — items close when the schema
> change is live in both bases and the probe confirms it.

**Reconciled against the Airtable team's v2 guide, 2026-08-17.** Items 0–12 carry their `v0.2` numbers so the
views stay traceable; 13–16 are new in `v0.3`; 17–27 were opened by live evidence.
...
| 1 | `Photos` field type = URL/long-text (not Attachment); must artifact links be publicly reachable? | Airtable | ✅ **CLOSED 2026-08-23** — live probe: `Photos` is `url`, matches §7. Link *reachability* remains open as gap B / item 12. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §3. |
| 2 | `Airtable … ID` fields: plain text or link-to-record? (LabOS proposes text) | Airtable | ✅ **CLOSED 2026-08-23** — live probe: all four are `singleLineText`, exactly as LabOS proposed. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §3. |
| 3 | **Read-side parameter structure: discrete fields or versioned JSON? (§9.1)** | Airtable / Luis | 🟡 **ANSWERED 2026-08-23, with a defect.** Neither option: it is **row-per-parameter** on `Protocol Sections` (`Section Name` + free-text `Value`). Addressable, so **W3 is unblocked on structure** — but untyped, and the sample data is **wrong** (new item **19**). LabOS still asks for `Required Value` + `Required Unit`. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §5. **2026-08-31 — they asked *us* to specify the typed representation.** Spec proposed in `correspondence/airtable-team-questions-2026-08-31.md` §2 (5 typed fields + `Requirement Kind`, `Value` untouched); pending their acceptance, at which point 3, 15 and 23 close together. **Sharpened 2026-08-31 by real-data evidence (`../evidence/labos-real-data-types-2026-08-31.md`):** LabOS derives 14+ test stages from the design-pressure pair, so the read-side dependency is **two numbers in PSF**, not a requirement model — no ranges, cycle sequences or hold times are read at all. |
...
| 5 | `Impact Result` option set — fixed list or free text? | Airtable / Luis | ✅ **CLOSED 2026-08-23** — live probe: `Impact Result` is `singleLineText`, free text. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §3. |
...
| 8 | Confirm the two-write lifecycle (`In Progress` then terminal) is acceptable | Airtable | 🟠 Open — now **coupled to 13**. A single `Test Date` only works cleanly if it means *start*. **2026-08-31 — confirmation re-requested** alongside their `Test Date` change; a single `Test Date` only works if it means *start*. See `correspondence/airtable-team-questions-2026-08-31.md` §4. |
...
| 10 | Rotated PAT with `data.records:read` + `:write` + `schema.bases:read`, delivered out-of-band | Airtable | ✅ **CLOSED 2026-08-23** — both PATs delivered (testing + production), stored in the gitignored `.env`. **Rotate after acceptance testing:** they arrived as plaintext email. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §1. |
| 11 | Verify the §5 null/blank table against the live base and add the evidence column | **LabOS** | ✅ **CLOSED 2026-08-23** — probe run against both bases, field-ID snapshots committed to `docs/labos-airtable/schema/`. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §2. |
| 12 | Reachable report/photo origin (gap B) before first production write | **LabOS** | 🟠 Open. |
| **13** | **The `Test Date` collapse.** v2 replaces `Testing Start Date` + `Testing End Date` with one `Test Date`. This removes test duration from the record entirely, and leaves the two-write lifecycle with no field that distinguishes "started at" from "finished at". **LabOS asks for both fields back.** Fallback if refused: `Test Date` means *start*, duration moves into `Complete LabOS JSON Response`, and item **8** must then be confirmed explicitly. | Airtable | 🔴 **NEW — blocking** **2026-08-31 — partially addressed.** They will make `Test Date` a `dateTime` (that closes **20**), but one instant still cannot hold both ends. `Testing End Date` re-requested; agreed fallback is `Test Date` = *start* with duration in the JSON valve. `correspondence/airtable-team-questions-2026-08-31.md` §4. |
| **14** | **`Corrects Attempt ID` + `Correction Reason` are absent.** These are the only two fields in §4 that the JSON valve cannot rescue, because Airtable automations must *branch* on them — a value buried in long text cannot be filtered, linked, or rolled up. Without them a correction and a genuine retest are indistinguishable in the data (§3.1), so any roll-up counting attempts or computing a pass rate is wrong in one of the two cases. | Airtable | 🔴 **NEW — blocking** **2026-08-31 — they propose replacing this field with the shared `LabOS Test ID`. Declined, with the cost stated.** Grouping is not supersession: the shared Test ID is already in every payload (§2) and answers a different question. Fallback offered — one Airtable formula field over `corrects_attempt_id` in the JSON valve, which makes this depend on **25**. **Still blocking.** `correspondence/airtable-team-questions-2026-08-31.md` §3. **Quantified 2026-08-31 against production:** of 507 real tests, **106 — one in five — already have more than one attempt**, and some have three. The ambiguity is present in live data now, not prospective. `../evidence/labos-real-data-types-2026-08-31.md` §6.3. |
| **15** | **Seven further fields absent**, all non-blocking because §6's JSON field can carry them: `Test Name`, `Abort Reason`, `Required Value`, `Required Unit`, `Cycles Required`, `Cycles Completed`, plus `Test Rig` / `LabOS Version` / `Result Rationale` and the six wall-snapshot fields (§4.2). LabOS asks for `Test Name` and `Abort Reason` as first-class fields — both are things a human reads off the row — and will carry the rest in JSON. Also: confirm the wall-snapshot omission is deliberate. | Airtable | 🟠 **NEW** **2026-08-31 — `Required Value` / `Required Unit` accepted in principle**; they requested the specification, supplied in `correspondence/airtable-team-questions-2026-08-31.md` §2. |
| **16** | **`Test Result` option spelling.** Their v2 worked example sends `"Passed"`; this contract specifies `Pass`. A single-select mismatch either 422s or silently creates a junk option, and the same risk applies to every select field. | Airtable | ✅ **CLOSED 2026-08-23 — against this contract.** The live base holds `Passed` / `Failed`; **their v2 example was right and this document was wrong.** §4.3 now records their spelling, and the envelope translates LabOS `Pass`→`Passed` at the wire boundary. See `docs/labos-airtable/evidence/live-probe-findings-2026-08-23.md` §6. |
...
| **23** | **How is a directional pressure pair represented in `Required Value`?** The proposal writes design pressure as **`+60/60`** — inward and outward magnitudes in one cell. A single numeric `Required Value` (item **15**) cannot hold it, so either `Required Value` needs a companion (`Required Value Inward` / `Required Value Outward`), or the pair needs an agreed encoding. Left unresolved, LabOS is back to parsing `+60/60` out of free text — which is exactly what item **15** exists to remove, and the parsing failure mode is a wrong pressure on a rig. | Airtable / Luis | 🔴 **NEW 2026-08-28 — raised in the verification report sent today** **2026-08-31 — answered by LabOS**: `Requirement Kind = Directional Pair` + `Required Value Inward` / `Required Value Outward` as positive magnitudes. `correspondence/airtable-team-questions-2026-08-31.md` §2.1. |
| **24** | **Unit vocabulary is unagreed, and the fields cannot enforce one.** This contract models `Unit` and `Deflection Unit` as **single selects** (§4.4); both are live as **`singleLineText`**. Their own sample row `recxZWiVa5Wuy0ZV6` writes `Deflection Unit = "Inches"` where this contract says `in`. Free text accepts both, so the column silently holds two spellings of one unit and nothing ever errors. **LabOS will match their incumbent value** until told otherwise. Preferred fix: make both fields single selects, which converts this from silent divergence into a loud rejection. | Airtable | 🔴 **NEW 2026-08-28 — blocking, silent-corruption class** **2026-08-31 — vocabulary specified** in answer to their question 1: `PSF PSI in mm lbf N cycles s impacts`, short form stored, display left to the interface. `in` wins over their `Inches`, and the single-select conversion is re-requested — which is what makes the divergence loud instead of silent. Adds `impacts` to §4.4 on ratification. **Corrected 2026-08-31:** one global list was wrong in both directions — §4.4 misses five units the lab measures in and carries `lbf` / `N`, which nothing produces. Resolution is **three scoped lists** (`Required Unit` · `Unit` · `Deflection Unit`); `kg` / `m/s` / `m²` / `cfm/ft²` / `minutes` stay in the JSON valve with per-value units. `../evidence/labos-real-data-types-2026-08-31.md` §3. §4.4 adds `impacts` and drops `lbf` / `N` on ratification. |
| **25** | **`Complete LabOS JSON Response` shape is unagreed.** Their sample carries `gauges` as a **list of objects** with a per-reading `unit`, plus `design_pressure` / `hold_time_seconds` / `observations`; §6 defines `gauges` as a **map** with the unit as a sibling column. **The question that decides it: does anything on their side parse this field?** If yes, LabOS adopts their shape. If it is human-readable only, §6 stands and we record that explicitly so no automation is later built on an assumption. | Airtable | 🔴 **NEW 2026-08-28** **2026-08-31 — re-asked, and now load-bearing:** it also gates the formula-field fallback offered for **14**. **2026-08-31 — reframed from style to completeness.** `deflections` stores **three** measurements per gauge (`max_deflection`, `permanent_deflection`, `recovery`), and permanent set is a required column of the IFET report; their sample JSON carries **one** number per gauge. Whichever shape wins must hold all three. `../evidence/labos-real-data-types-2026-08-31.md` §2.3. Also: `deflection_gauge` is inconsistent free text live — **20 distinct labels in two naming schemes** (`1-1`…`2-8` and `Gauge 1`…`Gauge 4`). Normalise before the JSON shape is agreed, or two naming schemes become two shapes in their base. `../evidence/labos-real-data-types-2026-08-31.md` §7.4. |
| **26** | **`Schema Version` semantics**, and whether `Retest Required` / `Testing Continued` are genuinely optional. Their row writes `Schema Version = "1.0"` (a payload version?) where LabOS sends the contract version (`0.3`), and omits both terminal fields that §4.5 requires. Low severity, but it is the difference between their sample being incomplete and our required-set being wrong. | Airtable | 🟠 **NEW 2026-08-28** |
| **27** | **⚠️ Deflection values are not plausible as inches — LabOS-side.** Live `deflections` holds `max_deflection` **−1280.91 → 1288.86** and `permanent_deflection` **−1274.17 → 1284.13**; a specimen does not deflect 1288 in. Either some rows carry raw gauge counts rather than converted inches, or a scale factor is missing on some path — the same class of defect as the PSI→PSF sensor bug of 2026-07-09 (§3.1). **This must be understood before LabOS writes `Deflection Value` / `Deflection Unit` to Airtable**, or the problem is exported into their base and becomes certification evidence there. The §2.3 structural argument (three measurements per gauge) is unaffected — it rests on the schema and the report contract, not on the values. Scope: which rows, whether a scale factor is missing, whether any reached a published report. `../evidence/labos-real-data-types-2026-08-31.md` §7.4. | **LabOS** | 🔴 **NEW 2026-08-31 — blocking the first real `Deflection Value` write** |
